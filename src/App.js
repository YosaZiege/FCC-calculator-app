...
function App() {
    ...
    if (!calculationData.length) {
      ...
      if (operators.includes(lastChar)) {
        ...
        if (lastChar === "." || currentInput.includes(".")) {
          // Do nothing if already a decimal point
        } else {
  ...
  const handleNumber = (value) => {
    if (!calculationData.length) {
      ...
      if (value === "0" && (calculationData === "0" || currentInput === "0")) {
        // Do nothing if adding 0 to existing 0
      } else {
        const lastChar = calculationData.charAt(calculationData.length - 1);
  ...
  const handleOperator = (value) => {
    if (String(calculationData).length) {
      ...
      if (lastCharisOp && value !== "-" || (lastCharisOp && secondLastCharisOp)) {
  ...
  const handleEquation = () => {
    try {
      ...
      // Handle error, for example, division by zero
      setFormula("Error");
